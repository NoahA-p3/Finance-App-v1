...
import { toDeviceHistory } from "@/lib/account-security";
...
export async function GET() {
  ...
  if (!sessions) {
    ...
    return NextResponse.json({ error: "Unable to load devices." }, { status: 502 });
  }

  const ownSessions = sessions.filter((session) => session.userId === authContext.user.id);
  return NextResponse.json({ devices: toDeviceHistory(ownSessions) });
}
