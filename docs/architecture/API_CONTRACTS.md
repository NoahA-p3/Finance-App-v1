...
- Company profile bootstrap flow:
  ...
  - Canonical ownership policy for finance entities is company-shared within the active company: authorization is membership + `company_id`; `user_id` is creator metadata (not per-user visibility gating).
